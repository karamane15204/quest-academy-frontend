  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#F5F5DC' }}>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Welcome Section */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2" style={{ color: '#3E2723' }}>Welcome back, Student!</h1>
          <p className="text-gray-600">Continue your learning journey and track your progress.</p>
        </div>

        {/* Stats Cards */}
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          <Card className="text-white" style={{ background: 'linear-gradient(135deg, #007D7A, #FF7F50)' }}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map((course) => (
                <Card key={course.id} className="hover:shadow-lg transition-shadow duration-300 bg-white border-gray-200">
                  <div className="aspect-video rounded-t-lg flex items-center justify-center" style={{ backgroundColor: '#F5B895' }}>
                    <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-md">
                      {course.type === 'Game-Based' ? <Play className="h-8 w-8" style={{ color: '#007D7A' }} /> : <Video className="h-8 w-8" style={{ color: '#007D7A' }} />}
                    </div>
                  </div>
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <CardTitle className="text-lg" style={{ color: '#3E2723' }}>{course.title}</CardTitle>
                      <Badge 
                        variant={course.status === 'Completed' ? 'default' : 'secondary'}
                        style={course.status === 'Completed' ? { backgroundColor: '#FF7F50', color: 'white' } : {}}
                      >
                        {course.status}
                      </Badge>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between text-sm mb-2">
                          <span style={{ color: '#3E2723' }}>Progress</span>
                          <span style={{ color: '#3E2723' }}>{course.progress}%</span>
                        </div>
                      ...
                      <Button 
                        ...
                        style={course.status === 'Completed' ? { color: '#3E2723', borderColor: '#3E2723' } : { backgroundColor: '#007D7A' }}
                      >
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 rounded-lg flex items-center justify-center" style={{ backgroundColor: '#F5B895' }}>
                          <FileText className="h-6 w-6" style={{ color: '#007D7A' }} />
                        </div>
                        <div>
                          <h3 className="font-semibold" style={{ color: '#3E2723' }}>{assignment.title}</h3>
                          <p className="text-sm text-gray-600">{assignment.course}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <Badge variant="outline" style={{ borderColor: '#FF7F50', color: '#FF7F50' }}>{assignment.type}</Badge>
          ...
          <TabsContent value="games" className="space-y-6">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              <Card className="hover:shadow-lg transition-shadow duration-300 bg-white border-gray-200">
                <div className="aspect-video rounded-t-lg flex items-center justify-center" style={{ backgroundColor: '#F5B895' }}>
                  <Play className="h-16 w-16" style={{ color: '#007D7A' }} />
                </div>
                <CardHeader>
                  <CardTitle style={{ color: '#3E2723' }}>Code Quest Adventure</CardTitle>
              ...
              <Card className="hover:shadow-lg transition-shadow duration-300 bg-white border-gray-200">
                <div className="aspect-video rounded-t-lg flex items-center justify-center" style={{ backgroundColor: '#F5B895' }}>
                  <Play className="h-16 w-16" style={{ color: '#FF7F50' }} />
                </div>
                <CardHeader>
                  <CardTitle style={{ color: '#3E2723' }}>Math Puzzle World</CardTitle>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {achievements.map((achievement, index) => (
                <Card 
                  ...
                  style={achievement.earned ? { borderColor: '#007D7A', backgroundColor: '#D4E6D4' } : {}}
                >
                  <CardContent className="p-6">
                    <div className="text-4xl mb-2">{achievement.icon}</div>
                    <h3 className="font-semibold mb-2" style={{ color: '#3E2723' }}>{achievement.title}</h3>
                    <Badge 
                      variant={achievement.earned ? 'default' : 'secondary'}
                      style={achievement.earned ? { backgroundColor: '#FF7F50', color: 'white' } : {}}
                    >
