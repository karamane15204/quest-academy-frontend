      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-4 bg-white border-gray-200">
            <TabsTrigger value="overview" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-700">Overview</TabsTrigger>
            <TabsTrigger value="courses" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-700">My Courses</TabsTrigger>
            <TabsTrigger value="students" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-700">Students</TabsTrigger>
            <TabsTrigger value="content" className="data-[state=active]:bg-gray-100 data-[state=active]:text-gray-700">Create Content</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle style={{ color: '#657165' }}>Recent Student Activity</CardTitle>
                  <CardDescription>Latest student progress and performance</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {recentStudents.map((student, index) => (
                    <div key={index} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: '#F7DDD2' }}>
                      <div>
                        <p className="font-medium" style={{ color: '#657165' }}>{student.name}</p>
                        <p className="text-sm text-gray-600">{student.course}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-medium" style={{ color: '#657165' }}>{student.progress}%</p>
                        <Badge 
                          variant={student.status === 'Excellent' ? 'default' : student.status === 'Good' ? 'secondary' : 'destructive'}
                          style={student.status === 'Excellent' ? { backgroundColor: '#E8B4B8', color: 'white' } : {}}
                        >
                          {student.status}
                        </Badge>
                      </div>
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle style={{ color: '#657165' }}>Pending Assignments</CardTitle>
                  <CardDescription>Assignments waiting for review</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {pendingAssignments.map((assignment, index) => (
                    <div key={index} className="p-3 rounded-lg" style={{ backgroundColor: '#F7DDD2' }}>
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-medium" style={{ color: '#657165' }}>{assignment.title}</h4>
                        <Badge variant="outline" style={{ borderColor: '#E8B4B8', color: '#E8B4B8' }}>{assignment.course}</Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">
                          {assignment.submissions} of {assignment.total} submitted
                        </span>
                        <Button size="sm" variant="outline" style={{ borderColor: '#657165', color: '#657165' }}>Review</Button>
                      </div>
                    </div>
                  ))}
                </CardContent>
          ...
          <TabsContent value="courses" className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold" style={{ color: '#657165' }}>My Courses</h2>
              <Button className="text-white hover:opacity-90" style={{ backgroundColor: '#99CDD8' }}>
                ...
                <Card key={course.id} className="hover:shadow-lg transition-shadow duration-300 bg-white border-gray-200">
                  <div className="aspect-video rounded-t-lg flex items-center justify-center" style={{ backgroundColor: '#F7DDD2' }}>
                    <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-md">
                      <BookOpen className="h-8 w-8" style={{ color: '#99CDD8' }} />
                    </div>
                  </div>
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      ...
                      <Badge 
                        ...
                        style={course.status === 'Active' ? { backgroundColor: '#E8B4B8', color: 'white' } : {}}
                      >
                        {course.status}
                      </Badge>
                    </div>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex justify-between text-sm">
                        <span style={{ color: '#657165' }}>Average Progress</span>
                        <span style={{ color: '#657165' }}>{course.progress}%</span>
                      </div>
                      <div className="flex space-x-2">
                        <Button className="flex-1 text-white hover:opacity-90" size="sm" style={{ backgroundColor: '#99CDD8' }}>Manage</Button>
                        <Button variant="outline" size="sm" style={{ borderColor: '#657165', color: '#657165' }}>Analytics</Button>
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          <TabsContent value="students" className="space-y-6">
            <Card className="bg-white border-gray-200">
              <CardHeader>
                <CardTitle style={{ color: '#657165' }}>Student Management</CardTitle>
                  ...
                  {recentStudents.map((student, index) => (
                    <div key={index} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                      <div className="flex items-center space-x-4">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold" style={{ background: 'linear-gradient(135deg, #99CDD8, #E8B4B8)' }}>
                      ...
                      <div className="text-right space-x-4">
                        <span className="text-sm" style={{ color: '#657165' }}>Progress: {student.progress}%</span>
                        <Badge 
                          variant={student.status === 'Excellent' ? 'default' : student.status === 'Good' ? 'secondary' : 'destructive'}
                          style={student.status === 'Excellent' ? { backgroundColor: '#E8B4B8', color: 'white' } : {}}
                        >
                        ...
                        <Button size="sm" variant="outline" style={{ borderColor: '#657165', color: '#657165' }}>View Details</Button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <TabsContent value="content" className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle style={{ color: '#657165' }}>Upload Learning Materials</CardTitle>
                  <CardDescription>Add videos, documents, and resources</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="content-title" style={{ color: '#657165' }}>Content Title</Label>
                    <Input id="content-title" placeholder="Enter content title" className="border-gray-300 focus:border-teal-500" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="content-type" style={{ color: '#657165' }}>Content Type</Label>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                    ...
                    <Button className="mt-4" variant="outline" style={{ borderColor: '#657165', color: '#657165' }}>Choose Files</Button>
                  </div>
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle style={{ color: '#657165' }}>Create Quiz/Assignment</CardTitle>
                  <CardDescription>Design interactive assessments</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="quiz-title" style={{ color: '#657165' }}>Quiz Title</Label>
                    <Input id="quiz-title" placeholder="Enter quiz title" className="border-gray-300 focus:border-teal-500" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="quiz-course" style={{ color: '#657165' }}>Course</Label>
                    <select className="w-full p-2 border border-gray-300 rounded-md focus:border-teal-500">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="quiz-type" style={{ color: '#657165' }}>Assessment Type</Label>
                    <select className="w-full p-2 border border-gray-300 rounded-md focus:border-teal-500">
